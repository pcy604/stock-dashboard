...
측정 (2018-06~2026-08, 편도 0.1%, 진입가 = 신호 주차 종가):
  ⚠️ 2026-08-23 재무조건에 **흑자전환**을 추가하고 분할 진입은 철회했다(아래는 반영분).
    L 단독 6칸  : 앞 15.5% / 뒤 40.9% / 전체 25.5%   MDD −33.2%
    S 단독 10칸 : 앞 33.6% / 뒤 20.7% / 전체 23.3%   MDD −49.4%
    L+S 50:50  : 앞 25.4% / 뒤 32.2% / **전체 24.5%**  MDD −41.0%
    SPY        : 앞 18.3% / 뒤 13.2% /     15.1%       MDD −31.8%
  진입을 1주 늦추면 23.8%, 2주 늦추면 16.8%. 현행 대조(재무조건 A)는 24.9/24.9/19.7 로
  **지연 안정성은 A 가 낫다** — 그럼에도 B 를 택한 이유는 아래 '흑자전환' 절 참조.
...
흑자전환 — 왜 넣었나 (2026-08-23 채택, 이 규칙의 핵심 변경)
  사용자 투자철학이 '구조적 성장주의 흑자전환 포착'인데, 2026-08-18 에 규칙⑥→L/S 로
  갈아탈 때 재무조건이 '영업익 8분기 신고점 or OPM 전분기比 +5%p' 로 좁아지면서
  **흑자전환(op_turn)이 빠졌다.** 규칙⑥에는 있던 조건이다.

  그게 TSLA·WDC 를 8년간 한 번도 못 잡던 원인이었다. 원자료로 확인:
    TSLA 2020-03-23  +20.3% · 거래량 0.73 · 영업익 $359M(흑자전환) · OPM 4.9%
      → 급등·거래량 통과. 그런데 8Q신고점 아님(2018Q3 $417M 이 더 높다),
        OPM 개선폭 +0.7%p(5%p 미달) → **재무조건에서만 탈락**
    TSLA 2020-06-29 · 2020-08-17 도 같은 사유
    WDC 2025-11-24 (OPM QoQ +28.9%p) · 2026-04-06 (8Q신고점) 은 재무·거래량 통과인데
      상승률이 +17.3% / +16.4% 라 **+20% 문턱**에 걸렸다 (이건 별개 문제로 남는다)
    NVDA 2022-11-07 은 OPM QoQ −15.1%(실적 급락) 시점인데 이후 52주 +196% —
      '좋아지는 것'만 보는 재무조건으로는 구조적으로 못 잡는다 (미해결)

  효과 (재무조건만 바꿈, 일괄 진입 · 정시):
    A 8Q신고 or OPM+5      CAGR 24.9% · MDD −39.8% · 초대박 24 · 3배+ 3 · TSLA 0 · WDC 0
    B A or 흑자전환         CAGR 25.5% · MDD −33.2% · 초대박 28 · 3배+ 5 · TSLA 3 · WDC 1
    C 흑자전환만            CAGR 19.9% · MDD −37.1% · 최대 수익 +1,507%
    D B + OPM바닥반등       CAGR 10.6% — 바닥반등은 넣으면 안 된다
  실제 장부 변화: TWST 를 2026-08-17 이 아니라 **2025-10-20 에 잡는다**(10개월 일찍,
  44주 보유 +326.8%).

  ⚠️ 정직하게 — B 는 지연 검증에서 A 에 진다:
    A 24.9 / 24.9 / 19.7   B 25.5 / 23.8 / 16.8   (정시 / 1주 / 2주 지연)
  그럼에도 B 를 택한 이유는 판정 축이 CAGR 안정성이 아니라 **'시대의 주도주를
  잡았나'** 이기 때문이다(2026-08-23 사용자 결정). A 는 TSLA·WDC 를 8년간 0회 잡는다.

분할 진입 — 넣었다가 철회했다 (2026-08-22 채택 → 08-23 철회)
...
필터 완화 × 반복 발생 — 포착은 되는데 수익이 안 남는다 (2026-08-23)
  형 목표: "승률 낮아도 되니 시대의 주도주(19-20 TSLA · 23-24 NVDA · 24-25 PLTR ·
  25-26 메모리)와 AXTI 같은 스몰캡 초대박을 잡자."

  먼저 현행 규칙의 성적표를 그 기준으로 냈다. 초대박(어떤 1년 창에서든 +300%↑)
  621종 중 신호가 뜬 건 436종(70%)이다. 그런데 형이 지명한 종목은 처참하다:
      TSLA 0회 · NVDA 0회 · WDC 0회 · SNDK 0회 (전부 미감지, 최대 1년 +881/305/1162%)
      AXTI 첫 신호 2021-03-08 → 그 뒤 1년 **−51%** (최대 +9,546%)
  대형 주도주는 아예 못 잡고, 소형은 잡되 잘못된 시점에 잡는다.

  그래서 필터를 완화하고 반복을 요구해봤다 (형 제안). **포착은 극적으로 는다:**
      현행 +20%·vol<1.5·1회    TSLA 0 · NVDA 0 · WDC 0 · MU 2 · PLTR 1 · AXTI 2
      완화 +15%·vol<2.0·2회+   TSLA 3 · NVDA 8 · WDC 3 · MU 4 · PLTR 3 · AXTI 1
      완화 + 재무조건 제거       TSLA 18 · NVDA 11 · WDC 10 · CVNA 28
  정시 성적도 좋다 — CAGR 24.9% → **34.6%**, 3배+ 거래 3건 → 8건,
  최대 수익 +532% → **+1,197%**, 중앙값 −1% → −10%(승률은 낮아진다).

  **그런데 지연 검증에서 죽는다.**
      완화 +15/2.0/2회+   정시 34.6% → 1주 14.2% → 2주 10.1%
      현행 +20/1.5/1회    정시 24.9% → 1주 24.9% → 2주 19.7%
  신호 주 종가에 정확히 사야만 성립하는 미시구조 착시다. +15% 완화가
  '이미 오른 것을 늦게 잡는' 성격이라 한 주만 밀려도 추격매수가 된다.

  "잡아서 오래 들고 가기"도 안 된다. 트레일을 풀수록 나빠진다:
      완화 신호 · 트레일 −30%  34.6%   (평균 보유 50주)
                      −40%  23.2%   (60주)
                      −50%   5.0%   (75주)  MDD −68.6%
                      −60%  12.2%  (112주)
      추세 불타기(+10%/+20%×4)·눌림목 4분할 모두 개선 없음 (−40%에서 17.9~19.6%)
      재무조건 제거(TSLA 18회 감지) 는 2.3~11.6% 로 최악
  → **오래 들고 있는 것 자체가 손해다.** 사후에 보면 TSLA·NVDA 를 오래 들었어야
    하지만, 사전에는 그게 TSLA 인지 DMC 인지 구분이 안 된다. 트레일을 풀면 둘 다
    오래 들게 되고, DMC(336주 보유·연 1.4%·SPY 대비 −256%p)가 자본을 죽인다.

  결론: **완화+반복은 '포착'이라는 목표는 달성하지만 '수익'으로 이어지지 않는다.**
  채택하지 않았다. 다시 열 조건 — 지연 1주에서도 현행을 넘거나, 포착한 종목을
  오래 들고도 자본이 죽지 않는 청산 규칙을 찾으면.

보유 종목이 시장을 못 이기는 문제 (2026-08-23 확인, 미해결)
  −30%/−40% 트레일은 **절대 하락만** 본다. 종목이 횡보하면 트레일에 영원히 안
  걸리고 칸만 차지한다. 현재 장부 실측 — 진입 이후 SPY 대비:
      GLPI  335주  +135.6% vs SPY +229.5%  → −93.9%p   (연 14.3% vs 20.5%)
      VICI  335주  +134.4% vs SPY +229.5%  → −95.1%p
      DMC   336주    +9.0% vs SPY +265.0%  → **−256.0%p**  (연 1.4% vs 22.3%)
      SPY 초과는 16종 중 10종 · L 은 6종 중 3종뿐
  상대강도 강제 청산을 재봤다:
      현행(트레일만)                    18.3% / MDD −27.2%
      진입이후 SPY 미달 · 26주마다       **19.7%** / −29.4%  (8년간 강제청산 6회)
      최근 26주 SPY 미달 · 13주마다       6.7% / −33.7%      ← 정상 조정까지 자른다
  유일하게 개선한 건 '진입 이후 누적 SPY 미달 · 26주 점검'인데 +1.4%p 를 6건이
  만들었다. 3단계 기준 ③(기여 구간)을 통과할 가능성이 낮아 보류했다.
...
RULES = {
    "L": dict(name="대형 주도주",
              ...
              text="시총 $2B+ · 주간 +20%↑ · 거래량 전주비 <1.5배 · "
                   "(영업익 8분기 신고점 OR OPM 전분기比 +5%p↑ OR 흑자전환)",
              exit="진입 후 주봉 종가 고점 대비 −30% → 그 주 종가 전량. 재신호 시 재진입",
              # 2026-08-22 눌림목 4분할을 채택했다가 2026-08-23 **철회**했다.
              #   재무조건에 흑자전환을 넣자 둘이 서로를 갉아먹는다:
              #     눌림목 분할 + 재무 A(현행)  18.3 / 18.4 / 15.1%  (정시/1주/2주 지연)
              #     눌림목 분할 + 재무 B(흑자↑) 18.0 / 14.3 /  8.1%
              #     일괄       + 재무 B        25.5 / 23.8 / 16.8%   ← 채택
              #   흑자전환 종목은 초기에 확 오른다. 눌림목을 기다리면 그 상승을 놓친다.
              #   대가: MDD 가 −27.2% → −33.2% 로 나빠진다. 그 대신 CAGR 7.2%p 와
              #   TSLA·WDC 포착을 얻었다.
              # '신호 추매'(같은 종목이 또 신호를 내면 추가)도 재고 기각했다 —
              #   보유 중 재신호가 8년에 17번뿐이라 추매 규칙이 아니라 '노출을 1/k 로
              #   줄이는 규칙'이 된다. 1/2×2 17.0% · 1/3×3 11.2% · 1/4×4 8.6%.
              tranche=None,
              tranche_text="일괄 진입 (분할·추매는 측정상 전부 손해였다)"),
    "S": dict(name="소형 대시세",
...
def load():
    ...
    d = pd.read_sql(f"""SELECT as_of,sym,name,close,ret_1w,adv_20d,marcap,period_end,
                               op_income,opm,rs_13w,rs_26w,dist_52w,per,psr,rev_yoy,ma20,op_turn
...
def gate_of(M, key):
    ...
    if r["mc_hi"]:
        g &= M["marcap"] < r["mc_hi"]
    if r["fund"]:
        # 2026-08-23: 흑자전환(op_turn)을 OR 로 추가했다. 08-18 에 규칙⑥→L/S 로
        # 갈아탈 때 재무조건이 '8Q신고점 or OPM+5%p' 로 좁아지면서 흑자전환이
        # 빠졌는데, 그게 사용자 투자철학의 핵심이고 실제로 TSLA·WDC 를 놓친 원인이었다.
        #   TSLA 2020-03-23 (+20.3% · 거래량 0.73) 은 급등·거래량을 통과했는데
        #   영업익 $359M 흑자전환이었는데도 8Q신고점이 아니고(2018Q3 $417M 이 더 높음)
        #   OPM 개선폭이 +0.7%p 라 재무조건에서만 탈락했다.
        #   WDC 2025-11-24·2026-04-06 도 재무·거래량 통과인데 상승률이 +17.3/+16.4% 라
        #   +20% 문턱에 걸렸다(이건 별개 문제로 남는다).
        g &= (M["F_HI8"] == 1) | (M["F_OPM"] == 1) | (M["op_turn"] == 1)
...
def build():
    ...
    M = matrices(d, ["close", "ret_1w", "adv_20d", "marcap", "vw", "F_HI8", "F_OPM",
                     "ma20", "op_turn"])   # op_turn = 흑자전환(2026-08-23 재무조건에 추가)
