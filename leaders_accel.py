...
워크포워드 검증 (2026-09-07 · leaders_accel_wf.py)
  앞 2년으로 급등 문턱을 고르고 뒤 1년에 그대로 적용했다. 뒤 1년은 규칙을 만들 때
  보지 않은 구간이다. '대조'는 문턱을 고르지 않고 항상 +10%(채택값)를 쓴 경우다.
    검증 구간          고른 문턱   그 문턱 알파   대조(+10%) 알파   대조 평균
    2020-06~2021-06     +25%        +69.0%          +13.9%        +36.9%
    2021-06~2022-06     +25%         +1.6%           −4.3%        −13.8%
    2022-06~2023-06     +25%        −10.9%           −8.6%         +8.3%
    2023-06~2024-06      +5%         −9.2%           −6.5%        +17.2%
    2024-06~2025-06     +25%        +22.0%          +16.8%        +36.0%
  · **채택 규칙(+10%)은 5분할 중 2개에서만 시장을 이겼다.**
    2021-06~2024-06 은 3년 연속 마이너스다. 인샘플 알파 +12.8% 는 사실상
    2020-21 과 2024-25 두 구간이 만든다. **회복장 전용 규칙으로 읽어야 한다.**
  · **모든 분할에서 중앙값이 시장에 진다**(−2.6 ~ −32.0%p). 평균이 양수인 건
    소수 대박이 끌어올리기 때문이다. 이 규칙을 쓴다는 건 대부분의 신호가 시장에
    지는 것을 받아들이고 꼬리를 기다린다는 뜻이다.
  · 문턱을 앞 구간 성적으로 고르면 5번 중 4번 +25% 를 고르고, 대조군을 3/5 로
    이긴다. 그런데 +25% 로 올리면 신호가 1,393건 → 139건으로 줄고 주도주를 놓친다
    (NVDA 13→1회 · TSLA 21→8회). 포착을 택한 것이 +10% 다.
  · 이전 L/S 규칙은 같은 잣대에서 1/5 였다. 이 규칙이 낫지만 2/5 를 '검증됐다'고
    부를 수는 없다. **화면에 성적을 그대로 띄우고 규칙은 유지한다**(2026-09-07 결정).
  ⚠️ 이 검증도 완전한 아웃오브샘플이 아니다. 가속의 정의(영업익을 매출로 스케일)와
     안전장치(매출 $10M · |가속| ≤ 10)는 전 구간을 보고 정했다. 그 선택은 워크포워드
     밖에 있다. 상장폐지 종목이 없는 것도 TRAIN·TEST 양쪽에 같은 낙관 편향을 준다.
