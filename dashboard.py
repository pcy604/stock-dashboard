...
# 2026-08-18 전면 교체. 규칙⑥(RS13>1.5 & OPM>0 & $2B+)은 시총 데이터를 고치고
    ...
    if not _ab:
        ...
        st.caption("구간을 갈라 각각 검증했다. **두 국면 모두에서 SPY를 앞선다** — "
                   ...
                   "⚠️ 2026-08-22 부터 L 은 **눌림목 4분할 진입**이라 이전 공시치"
                   "(L 24.9% · L+S 24.5%)보다 CAGR 이 낮고 MDD 가 좋다 — 아래 참조.")
        ...
        for _t, _k in zip(_rt, ('L', 'S')):
            with _t:
                ...
                if _k == 'L':
                    st.warning(
                        "**분할 진입은 수익 규칙이 아니라 보험이다.** 2026-08-22 측정 결과 "
                        "CAGR 은 24.9% → **18.3%** 로 6.6%p 낮아지고 MDD 는 −39.8% → "
                        "**−27.2%** 로 12.6%p 좋아진다. 이 MDD 우위는 진입을 1·2주 늦춰도 "
                        "일관되게 남는 유일한 효과다(CAGR 은 반대로 일괄이 세 시점 전부 이긴다). "
                        "2026-06 손실이 '한 번에 실은 것'에서 왔으므로 그 대가를 치르기로 한 "
                        "선택이다. **수익이 낮다고 나중에 흔들리지 않기 위해 여기 적어 둔다.** "
                        "S 에는 안 쓴다 — 분할이 CAGR 만 깎고(23.3%→12.4~20.9%) MDD 는 "
                        "그대로였다(−49.4%→−50.1%).", icon="🛡")
                _m = st.columns(4)
                ...
                if _bk:
                    ...
                    if _rr.get('tranche'):
                        st.caption(
                            "칸은 다 찼는데 현금이 남아 있다면 **분할 진입 때문이다** — "
                            "1/4 만 사고 눌림목을 기다리는 중인 종목이 있다는 뜻이다. "
                            "이 남은 현금이 MDD 를 −39.8%에서 −27.2%로 낮추는 실체다.")
                    _bp = _bk['positions']
                    if _bp:
                        st.dataframe(pd.DataFrame([{
